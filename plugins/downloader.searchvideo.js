...
// PERBAIKAN FATAL: Menyesuaikan import dengan package.json milikmu (lily-baileys)
...
// =========================================================================
// GLOBAL: Mengirim Gambar + Tombol Native (Untuk Pinterest & YTS)
// =========================================================================
async function sendButtonWithImage(conn, jid, imageUrl, caption, footerText, buttons, quoted) {
    try {
        ...
        console.error("Gagal Render Tombol Native:", e.message);
        // FALLBACK: Jika WhatsApp pengguna tidak mendukung tombol native
        let fallbackCaption = caption + '\n\n💡 _Balas pesan ini dengan teks berikut untuk melanjutkan:_\n';
        buttons.forEach(btn => fallbackCaption += `> Ketik: *${btn.id}* (untuk ${btn.text})\n`);
...
async function showYts(conn, m, jid) {
    ...
    let caption = `📺 *YouTube Search:* ${ytData.query}\n` +
                  ...
                  `🎬 *Video:* ${index + 1}/${total}\n` +
                  `⏳ _Sesi pencarian berlaku 5 menit_`;
...
let handler = async (m, { conn, args, text, usedPrefix, command }) => {
  ...
  switch (command) {
    ...
    // ================== PINTEREST ==================
    case 'pinterest': {
      if (!text) throw `*🚩 Format Pencarian:* ${usedPrefix}${command} shiroko\n*🚩 Format Download:* ${usedPrefix}${command} https://id.pinterest.com/pin/1234567890/`;
...
handler.before = async (m, { conn }) => {
  ...
  // Menangkap balasan dari nativeFlow (tombol WA)
