let handler  = async (m, { conn, command, args, usedPrefix, owner }) => {
    ...
    // ==========================================
    // DAFTAR SEMUA ITEM BESERTA RANGE STOCK NYA
    // ==========================================
    const itemStockRange = {
        'limit':         [40000, 60000],
        'pet':           [20000, 30000],
        'gardenboxs':    [8000,  12000],
        'bensin':        [80000, 120000],
        'weapon':        [4000,  6000],
        'obat':          [40000, 60000],
        'tiketcoin':     [120000,180000],
        'healtmonster':  [15000, 25000],
        'pancingan':     [10000, 20000],
        'bibitpisang':   [80000, 120000],
        'bibitanggur':   [80000, 120000],
        'bibitmangga':   [80000, 120000],
        'bibitjeruk':    [80000, 120000],
        'bibitapel':     [80000, 120000],
        'bibitpadi':     [80000, 120000],
        'bibitgandum':   [80000, 120000],
        'bibitwortel':   [80000, 120000],
        'bibitkentang':  [80000, 120000],
        'bibitsingkong': [80000, 120000],
        'bibitubijalar': [80000, 120000],
        'bibittebu':     [80000, 120000],
        'potion':        [40000, 60000],
        'sampah':        [400000,600000],
        'string':        [15000, 25000],
        'botol':         [120000,180000],
        'kaleng':        [120000,180000],
        'kardus':        [120000,180000],
        'sword':         [4000,  6000],
        'kayu':          [250000,350000],
        'batu':          [250000,350000],
        'coal':          [120000,180000],
        'iron':          [40000, 60000],
        'berlian':       [8000,  12000],
        'emasbatang':    [4000,  6000],
        'emasmentah':    [15000, 22000],
        'pasir':         [18000, 26000],
        'uranium':       [13000, 20000],
        'pisang':        [65000, 95000],
        'anggur':        [65000, 95000],
        ...
        'jeruk':         [65000, 95000],
        'apel':          [65000, 95000],
        'makananpet':    [15000, 25000],
        'makanannaga':   [4000,  6000],
        'makanankyubi':  [4000,  6000],
        'makanangriffin':[8000,  12000],
        'makananphonix': [8000,  12000],
        'makanancentaur':[4000,  6000],
        'aqua':          [80000, 120000],
        'susu':          [65000, 95000],
        'madu':          [17000, 25000],
        'umpan':         [80000, 120000],
        'airmineral':    [16000, 24000],
        'tehbotol':      [20000, 28000],
        'nescafe':       [14000, 21000],
        'ultramilk':     [17000, 25000],
        'jusanggur':     [15000, 22000],
        'jusapel':       [19000, 27000],
        'jusjeruk':      [13000, 20000],
        'jusmangga':     [21000, 29000],
        'juspisang':     [18000, 25000],
    };

    // COMMAND .resetstock — HANYA OWNER
    if (/^resetstock$/i.test(command)) {
        if (!owner.includes(m.sender)) return conn.reply(m.chat, '❌ Hanya owner yang bisa reset stock!', m);
        for (let key in itemStockRange) {
            let [min, max] = itemStockRange[key];
            let randomStock = Math.floor(Math.random() * (max - min + 1)) + min;
            global.db.data.market[key] = { stock: randomStock };
        }
        return conn.reply(m.chat, '✅ *RESET STOCK BERHASIL!*\nSemua stock item telah di-random ulang secara acak!', m);
    }

    function getMarketPrice(itemKey, baseBeli, baseJual) {
        let [min, max] = itemStockRange[itemKey];
        // Kalau belum ada di DB, random dulu
        ...
        let baseStock = Math.floor((min + max) / 2);
    ...
    let dataLimit = getMarketPrice('limit', 15, 10); let Blimit = dataLimit.beli; let Slimit = dataLimit.jual; let statusLimit = dataLimit.status;
    let dataPet = getMarketPrice('pet', 11, 5); let Bpet = dataPet.beli; let Spet = dataPet.jual; let statusPet = dataPet.status;
    let dataGarden = getMarketPrice('gardenboxs', 40000, 15000); let Bgardenboxs = dataGarden.beli; let Sgardenboxs = dataGarden.jual; let statusGarden = dataGarden.status;
    let dataBensin = getMarketPrice('bensin', 6000, 2000); let BBensin = dataBensin.beli; let SBensin = dataBensin.jual; let statusBensin = dataBensin.status;
    let dataWeap = getMarketPrice('weapon', 120000, 35000); let BWeap = dataWeap.beli; let SWeap = dataWeap.jual; let statusWeap = dataWeap.status;
    let dataObat = getMarketPrice('obat', 10000, 2500); let BObat = dataObat.beli; let SObat = dataObat.jual; let statusObat = dataObat.status;
    let dataTiketCoin = getMarketPrice('tiketcoin', 5, 1); let Btiketcoin = dataTiketCoin.beli; let Stiketcoin = dataTiketCoin.jual; let statusTiketCoin = dataTiketCoin.status;
    let dataHealtMonster = getMarketPrice('healtmonster', 25000, 6000); let Bhealtmonster = dataHealtMonster.beli; let Shealtmonster = dataHealtMonster.jual; let statusHealtMonster = dataHealtMonster.status;
    let dataPancingan = getMarketPrice('pancingan', 35000, 8000); let Bpancingan = dataPancingan.beli; let Spancingan = dataPancingan.jual; let statusPancingan = dataPancingan.status;

    let dataBibitPisang = getMarketPrice('bibitpisang', 550, 50); let Bbibitpisang = dataBibitPisang.beli; let Sbibitpisang = dataBibitPisang.jual; let statusBibitPisang = dataBibitPisang.status;
    let dataBibitAnggur = getMarketPrice('bibitanggur', 550, 50); let Bbibitanggur = dataBibitAnggur.beli; let Sbibitanggur = dataBibitAnggur.jual; let statusBibitAnggur = dataBibitAnggur.status;
    let dataBibitMangga = getMarketPrice('bibitmangga', 550, 50); let Bbibitmangga = dataBibitMangga.beli; let Sbibitmangga = dataBibitMangga.jual; let statusBibitMangga = dataBibitMangga.status;
    let dataBibitJeruk = getMarketPrice('bibitjeruk', 550, 50); let Bbibitjeruk = dataBibitJeruk.beli; let Sbibitjeruk = dataBibitJeruk.jual; let statusBibitJeruk = dataBibitJeruk.status;
    let dataBibitApel = getMarketPrice('bibitapel', 550, 50); let Bbibitapel = dataBibitApel.beli; let Sbibitapel = dataBibitApel.jual; let statusBibitApel = dataBibitApel.status;
    let dataPadi = getMarketPrice('bibitpadi', 400, 80); let Bpadi = dataPadi.beli; let Spadi = dataPadi.jual; let statusPadi = dataPadi.status;
    let dataGandum = getMarketPrice('bibitgandum', 450, 100); let Bgandum = dataGandum.beli; let Sgandum = dataGandum.jual; let statusGandum = dataGandum.status;
    let dataWortel = getMarketPrice('bibitwortel', 500, 120); let Bwortel = dataWortel.beli; let Swortel = dataWortel.jual; let statusWortel = dataWortel.status;
    let dataKentang = getMarketPrice('bibitkentang', 600, 140); let Bkentang = dataKentang.beli; let Skentang = dataKentang.jual; let statusKentang = dataKentang.status;
    let dataSingkong = getMarketPrice('bibitsingkong', 350, 70); let Bsingkong = dataSingkong.beli; let Ssingkong = dataSingkong.jual; let statusSingkong = dataSingkong.status;
    let dataUbiJalar = getMarketPrice('bibitubijalar', 375, 75); let Bubijalar = dataUbiJalar.beli; let Subijalar = dataUbiJalar.jual; let statusUbiJalar = dataUbiJalar.status;
    let dataTebu = getMarketPrice('bibittebu', 550, 130); let Btebu = dataTebu.beli; let Stebu = dataTebu.jual; let statusTebu = dataTebu.status;

    let dataPotion = getMarketPrice('potion', 20000, 100); let potion = dataPotion.beli; let Spotion = dataPotion.jual; let statusPotion = dataPotion.status;
    let dataSampah = getMarketPrice('sampah', 120, 5); let Bsampah = dataSampah.beli; let Ssampah = dataSampah.jual; let statusSampah = dataSampah.status;
    let dataString = getMarketPrice('string', 50000, 5000); let Bstring = dataString.beli; let Sstring = dataString.jual; let statusString = dataString.status;
    let dataBotol = getMarketPrice('botol', 300, 50); let Bbotol = dataBotol.beli; let Sbotol = dataBotol.jual; let statusBotol = dataBotol.status;
    let dataKaleng = getMarketPrice('kaleng', 400, 100); let Bkaleng = dataKaleng.beli; let Skaleng = dataKaleng.jual; let statusKaleng = dataKaleng.status;
    let dataKardus = getMarketPrice('kardus', 400, 50); let Bkardus = dataKardus.beli; let Skardus = dataKardus.jual; let statusKardus = dataKardus.status;
    let dataSword = getMarketPrice('sword', 150000, 15000); let Bsword = dataSword.beli; let Ssword = dataSword.jual; let statusSword = dataSword.status;

    let dataKayu = getMarketPrice('kayu', 1000, 400); let Bkayu = dataKayu.beli; let Skayu = dataKayu.jual; let statusKayu = dataKayu.status;
    let dataBatu = getMarketPrice('batu', 500, 100); let Bbatu = dataBatu.beli; let Sbatu = dataBatu.jual; let statusBatu = dataBatu.status;
    let dataCoal = getMarketPrice('coal', 1500, 1000); let Bcoal = dataCoal.beli; let Scoal = dataCoal.jual; let statusCoal = dataCoal.status;
    let dataIron = getMarketPrice('iron', 20000, 5000); let Biron = dataIron.beli; let Siron = dataIron.jual; let statusIron = dataIron.status;
    let dataBerlian = getMarketPrice('berlian', 150000, 10000); let Bberlian = dataBerlian.beli; let Sberlian = dataBerlian.jual; let statusBerlian = dataBerlian.status;
    let dataEmasBatang = getMarketPrice('emasbatang', 250000, 10000); let Bemasbatang = dataEmasBatang.beli; let Semasbatang = dataEmasBatang.jual; let statusEmasBatang = dataEmasBatang.status;
    let dataEmasMentah = getMarketPrice('emasmentah', 866490, 700000); let Bemasmentah = dataEmasMentah.beli; let Semasmentah = dataEmasMentah.jual; let statusEmasMentah = dataEmasMentah.status;
    let dataPasir = getMarketPrice('pasir', 250000, 180000); let Bpasir = dataPasir.beli; let Spasir = dataPasir.jual; let statusPasir = dataPasir.status;
    let dataUranium = getMarketPrice('uranium', 35000, 25000); let Buranium = dataUranium.beli; let Suranium = dataUranium.jual; let statusUranium = dataUranium.status;

    let dataPisang = getMarketPrice('pisang', 5500, 100); let Bpisang = dataPisang.beli; let Spisang = dataPisang.jual; let statusPisang = dataPisang.status;
    let dataAnggur = getMarketPrice('anggur', 5500, 150); let Banggur = dataAnggur.beli; let Sanggur = dataAnggur.jual; let statusAnggur = dataAnggur.status;
    let dataMangga = getMarketPrice('mangga', 4600, 150); let Bmangga = dataMangga.beli; let Smangga = dataMangga.jual; let statusMangga = dataMangga.status;
    let dataJeruk = getMarketPrice('jeruk', 6000, 300); let Bjeruk = dataJeruk.beli; let Sjeruk = dataJeruk.jual; let statusJeruk = dataJeruk.status;
    let dataApel = getMarketPrice('apel', 5500, 400); let Bapel = dataApel.beli; let Sapel = dataApel.jual; let statusApel = dataApel.status;
    let dataMakananPet = getMarketPrice('makananpet', 50000, 500); let Bmakananpet = dataMakananPet.beli; let Smakananpet = dataMakananPet.jual; let statusMakananPet = dataMakananPet.status;
    let dataMakananNaga = getMarketPrice('makanannaga', 150000, 10000); let Bmakanannaga = dataMakananNaga.beli; let Smakanannaga = dataMakananNaga.jual; let statusMakananNaga = dataMakananNaga.status;
    let dataMakananKyubi = getMarketPrice('makanankyubi', 150000, 10000); let Bmakanankyubi = dataMakananKyubi.beli; let Smakanankyubi = dataMakananKyubi.jual; let statusMakananKyubi = dataMakananKyubi.status;
    let dataMakananGriffin = getMarketPrice('makanangriffin', 80000, 5000); let Bmakanangriffin = dataMakananGriffin.beli; let Smakanangriffin = dataMakananGriffin.jual; let statusMakananGriffin = dataMakananGriffin.status;
    let dataMakananPhonix = getMarketPrice('makananphonix', 80000, 5000); let Bmakananphonix = dataMakananPhonix.beli; let Smakananphonix = dataMakananPhonix.jual; let statusMakananPhonix = dataMakananPhonix.status;
    let dataMakananCentaur = getMarketPrice('makanancentaur', 150000, 10000); let Bmakanancentaur = dataMakananCentaur.beli; let Smakanancentaur = dataMakananCentaur.jual; let statusMakananCentaur = dataMakananCentaur.status;

    let dataAqua = getMarketPrice('aqua', 5000, 1000); let Baqua = dataAqua.beli; let Saqua = dataAqua.jual; let statusAqua = dataAqua.status;
    let dataSusu = getMarketPrice('susu', 6000, 1200); let Bsusu = dataSusu.beli; let Ssusu = dataSusu.jual; let statusSusu = dataSusu.status;
    let dataMadu = getMarketPrice('madu', 64000, 50000); let Bmadu = dataMadu.beli; let Smadu = dataMadu.jual; let statusMadu = dataMadu.status;
    let dataUmpan = getMarketPrice('umpan', 1500, 100); let Bumpan = dataUmpan.beli; let Sumpan = dataUmpan.jual; let statusUmpan = dataUmpan.status;
    let dataAirMineral = getMarketPrice('airmineral', 9900, 7000); let Bairmineral = dataAirMineral.beli; let Sairmineral = dataAirMineral.jual; let statusAirMineral = dataAirMineral.status;
    let dataTehBotol = getMarketPrice('tehbotol', 9600, 7000); let Btehbotol = dataTehBotol.beli; let Stehbotol = dataTehBotol.jual; let statusTehBotol = dataTehBotol.status;
    let dataNescafe = getMarketPrice('nescafe', 14400, 10000); let Bnescafe = dataNescafe.beli; let Snescafe = dataNescafe.jual; let statusNescafe = dataNescafe.status;
    let dataUltraMilk = getMarketPrice('ultramilk', 10000, 7500); let Bultramilk = dataUltraMilk.beli; let Sultramilk = dataUltraMilk.jual; let statusUltraMilk = dataUltraMilk.status;
    let dataJusAnggur = getMarketPrice('jusanggur', 12000, 9000); let Bjusanggur = dataJusAnggur.beli; let Sjusanggur = dataJusAnggur.jual; let statusJusAnggur = dataJusAnggur.status;
    let dataJusApel = getMarketPrice('jusapel', 12300, 9200); let Bjusapel = dataJusApel.beli; let Sjusapel = dataJusApel.jual; let statusJusApel = dataJusApel.status;
    let dataJusJeruk = getMarketPrice('jusjeruk', 12600, 9400); let Bjusjeruk = dataJusJeruk.beli; let Sjusjeruk = dataJusJeruk.jual; let statusJusJeruk = dataJusJeruk.status;
    let dataJusMangga = getMarketPrice('jusmangga', 12900, 9600); let Bjusmangga = dataJusMangga.beli; let Sjusmangga = dataJusMangga.jual; let statusJusMangga = dataJusMangga.status;
    let dataJusPisang = getMarketPrice('juspisang', 13300, 10000); let Bjuspisang = dataJusPisang.beli; let Sjuspisang = dataJusPisang.jual; let statusJusPisang = dataJusPisang.status;
...
handler.help = ['shop <buy|sell> <item> <jumlah>', 'shop semua', 'shop <kebutuhan|bibit|barang|alam|perhiasan|crate|makanan|minuman|jus>', 'resetstock']
handler.tags = ['rpg']
handler.command = /^(shop|toko|buy|beli|sell|jual|resetstock)$/i
